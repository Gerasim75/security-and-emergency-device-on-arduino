...
void SetControlledRelay(String & result)
{
  ...
  case 30:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 3 OFF"));                  // Отправляем уведомление о выключении реле 3
    break;
  case 31:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 3 ON"));                   // Отправляем уведомление о включении реле 3
    break;
  case 40:
   ...
   shiftOut(dataPin, clockPin, LSBFIRST, byteToSend1); // Записываем первый байт
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 4 OFF"));                  // Отправляем уведомление о выключении реле 4
    break;
  case 41:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 4 ON"));                   // Отправляем уведомление о включении реле 4
    break;
  case 50:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 5 OFF"));                  // Отправляем уведомление о выключении реле 5
    break;
  case 51:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 5 ON"));                   // Отправляем уведомление о включении реле 5
    break;
  case 60:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 6 OFF"));                  // Отправляем уведомление о выключении реле 6
    break;
  case 61:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 6 ON"));                   // Отправляем уведомление о включении реле 6
    break;
  case 70:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 7 OFF"));                  // Отправляем уведомление о выключении реле 7
    break;
  case 71:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 7 ON"));                   // Отправляем уведомление о включении реле 7
    break;
  case 80:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 8 OFF"));                  // Отправляем уведомление о выключении реле 8
    break;
  case 81:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 8 ON"));                   // Отправляем уведомление о включении реле 8
    break;
  case 90:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 9 OFF"));                  // Отправляем уведомление о выключении реле 9
    break;
  case 91:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 9 ON"));                   // Отправляем уведомление о включении реле 9
    break;
    ////////////////////////////////////////////////
  case 100:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 10 OFF"));                 // Отправляем уведомление о выключении реле 10
    break;
  case 101:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 10 ON"));                  // Отправляем уведомление о включении реле 10
    break;
  case 110:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 11 OFF"));                 // Отправляем уведомление о выключении реле 11
    break;
  case 111:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 11 ON"));                  // Отправляем уведомление о включении реле 11
    break;
  case 120:
   ...
   shiftOut(dataPin, clockPin, LSBFIRST, byteToSend1); // Записываем первый байт
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 12 OFF"));                 // Отправляем уведомление о выключении реле 12
    break;
  case 121:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 12 ON"));                  // Отправляем уведомление о включении реле 12
    break;
  case 130:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 13 OFF"));                 // Отправляем уведомление о выключении реле 13
    break;
  case 131:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 13 ON"));                  // Отправляем уведомление о включении реле 13
    break;
  case 140:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 14 OFF"));                 // Отправляем уведомление о выключении реле 14
    break;
  case 141:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 14 ON"));                  // Отправляем уведомление о включении реле 14
    break;
  case 150:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 15 OFF"));                 // Отправляем уведомление о выключении реле 15
    break;
  case 151:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 15 ON"));                  // Отправляем уведомление о включении реле 15
    break;
  case 160:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 16 OFF"));                 // Отправляем уведомление о выключении реле 16
    break;
  case 161:
    ...
    shiftOut(dataPin, clockPin, LSBFIRST, byteToSend2); // Записываем второй байт
    digitalWrite(latchPin, HIGH);               // Закрываем защелку сдвигового регистра (активируем пины)
    SendSMS(F("Relay 16 ON"));                  // Отправляем уведомление о включении реле 16
    break;
  default:
    break;
  }
