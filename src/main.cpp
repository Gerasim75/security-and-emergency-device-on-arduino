...
void GetSensors()
{
  for(int i = 0; i < 6; i ++ )                           // Проходим по сенсорам
  {
    if(zona[i].ReadPin())                                // Если есть сработка
    {
      ...
      srabotki[counter_srabotki] = zona[i].adress_;      // Записываем адрес зоны в массив сработок
...
void AlarmMessages()
{
  if(flag_alarm)
  {
     ...
     {
     if(zona[srabotki[i]].alarm_ == true && zona[srabotki[i]].send_alarm_ == false)    // Если поднят флаг сработки датчика и уведомление не отправлялось
      {
          if(i == 0 && srabotki[i] == 0)                                                     // Если сработка в зоне 1
            {
               if(millis() - timer_zaderjka > time_zaderjka_off)                             // Если сработал таймер
                 {     
                    ActivateRelay(zona[srabotki[i]].pin_alarm_1_, zona[srabotki[i]].pin_alarm_2_); // Активируем реле тревоги
                    SendSMS(zona[srabotki[i]].message_alarm_);                               // Отправляем смс уведомление
                    if(!flag_tel)                                                            // Если еще не было звонка администратору
                    {
                      flag_tel = true;                                                       // Поднимаем флаг звонка  
                      Call(phones[0]);                                                       // Звоним администратору
                    }
                    zona[srabotki[i]].send_alarm_ = true;                                    // Поднимаем флаг отправленного уведомления
...
if((millis() - timer_balance) > time_balance)
  {
    ...
    float balance = GetBalans(kod_operatora);              // Извлекаем баланс
    delay(3000);
...
///////////////////////// 2  Выход клапан воды                           
    ...
    for(int i = 0; i < 6; i ++ )
    {
      zona[i].adress_ = i;                                    // Инициализируем адрес зоны
      switch (i)                                              // Инициализируем пины тревоги и уведомления о сработках
      {
      case 0:                                                // Для зоны 1 (охранная)
       strcpy(zona[i].message_alarm_,"Srabotka v zone 1!");     // Формируем уведомление о сработке
       zona[i].pin_alarm_1_ = 3;                                // При сработке задействуем реле тревоги
       break;
      case 1:                                                // Для зоны 2 (охранная)
       strcpy(zona[i].message_alarm_,"Srabotka v zone 2!");     // Формируем уведомление о сработке
       zona[i].pin_alarm_1_ = 3;                                // При сработке задействуем реле тревоги
       break;
      case 2:                                                // Для зоны 3 (охранная)
       strcpy(zona[i].message_alarm_,"Srabotka v zone 3!");     // Формируем уведомление о сработке  
       zona[i].pin_alarm_1_ = 3;                                // При сработке задействуем реле тревоги
       break;
      case 3:                                                // Для зоны 4 (аварийная "Пожар")
       strcpy(zona[i].message_alarm_,"Vozgoranie!!!");          // Формируем уведомление о сработке 
       zona[i].pin_alarm_1_ = 3;                                // При сработке задействуем реле тревоги
       zona[i].pin_alarm_2_ = 1;                                // При сработке задействуем реле выключения клапана газа
       break;
      case 4:                                                // Для зоны 5 (аварийная "Газ")
       strcpy(zona[i].message_alarm_,"Utechka gaza!!!");        // Формируем уведомление о сработке 
       zona[i].pin_alarm_1_ = 3;                                // При сработке задействуем реле тревоги
       zona[i].pin_alarm_2_ = 1;                                // При сработке задействуем реле выключения клапана газа
       break;
      case 5:                                                // Для зоны 6 (аварийная "Вода")
       strcpy(zona[i].message_alarm_,"Protechka vody!");        // Формируем уведомление о сработке 
       zona[i].pin_alarm_1_ = 3;                                // При сработке задействуем реле тревоги
       zona[i].pin_alarm_2_ = 2;                                // При сработке задействуем реле выключения клапана воды 
       break;
      }
    }
    InitialMacros();
  ...
  void InitialMacros()      
  {
    switch (macros_id)
    {
     case 0:
      for(int i = 0; i < 6; i ++ )
      {
        zona[i].status_ = macros_1[i];   // Активны все зоны
      }
      break;
     case 1:
      for(int i = 0; i < 6; i ++ )
      {
        zona[i].status_ = macros_2[i];   // Активны только аварийные зоны        
      }
      break;
     case 2:
      for(int i = 0; i < 6; i ++ )
      {
        zona[i].status_ = macros_3[i];   // Активны только охранные зоны        
      }
      break;
     case 3:
      for(int i = 0; i < 6; i ++ )
      {
        zona[i].status_ = macros_4[i];   // Активны аварийные зоны и две охранные      
      }
      break;  
    default:
      for(int i = 0; i < 6; i ++ )
