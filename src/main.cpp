...
#define KeyboardPin 18    // Вход клавиатура 
#define VoltPin 17        // Вход сеть
#define BuzzerPin 6       // На плате
#define ButtonPin 16      // На плате
...
void GetKeyboard()
{
  if(digitalRead(KeyboardPin) == LOW)                               // Если пропал сигнал с клавиатуры
  {
    delay(1500);
    if(digitalRead(KeyboardPin) == HIGH)                            // Если сигнал появился через 1,5 сек, сигнал валидный (смена режима)
    { 
...
void GetIncomingCall()    
{
  ...
  else
  {
    if(flag_admin)                                 // Если звонит админ
    {
      ...
      if (inner_phone.length()  >=  7 && digitalRead(ButtonPin) == HIGH)  // и нажата кнопка на плате
      {
